...
class AddYfthAcceptancePasswordResetPermission extends Migrator
{
    private const AUTH = 'yfth-user-role-management-fixture-password-reset';
    ...
    public function up()
    {
        ...
        $existing = $this->getAdapter()->fetchAll(
            'SELECT `id` FROM ' . $table . ' WHERE `unique_auth`=' . $this->quote(self::AUTH)
        );
        if (count($existing) > 1) {
            throw new RuntimeException('yfth_fixture_password_reset_permission_duplicate');
        }
        if ($existing) {
            return;
        }
        $row = [
            'pid' => (int)$page['id'], 'icon' => '', 'menu_name' => '重置验收测试账号密码', 'module' => 'admin',
            'controller' => 'v1.yfth.HqUserRole', 'action' => '', 'api_url' => 'yfth/user_role/fixture/password/reset',
            'methods' => 'POST', 'params' => '', 'sort' => 0, 'is_show' => 0, 'is_show_path' => 0,
