...
> 文档来源优先级固定为：
>
> 1. 当前最新发布范围：[YFTH_RELEASE_SCOPE_HQ_MALL_MEMBERSHIP_REFERRAL.md](YFTH_RELEASE_SCOPE_HQ_MALL_MEMBERSHIP_REFERRAL.md)。
> 2. 当前项目事实：[PROJECT_HANDOFF.md](PROJECT_HANDOFF.md) 顶部最新 `Current Fact Snapshot`。
> 3. 已实现模块的兼容架构文档，仅用于描述仓库现状与稳定边界。
> 4. 历史产品和差距文档：`PRODUCT_REQUIREMENTS.md`、原始 DOCX 及本文件历史正文。
>
> 低优先级文档不得覆盖高优先级文档；历史正文中的“当前”“下一步”“必须开发”等措辞只能按当时阶段理解。后续 Codex 开始任务前必须优先读取最新范围文档和交接文档顶部快照。
...
- Architecture-review P1 closure: headquarters `complete` no longer allows `confirmed -> completed` or `preparing -> completed`; express delivery must reach `shipped` before final consumption. The re-review's remaining self-pickup P1 is also closed: store `pickup_confirm` now accepts only `preparing`, so `confirmed -> pickup_confirm -> completed` is rejected without changing benefit/counter/event/audit state.
...
- Service-level real-flow coverage now includes claim idempotency, payload mismatch, non-owner/unavailable package rejection, user cancel boundaries, legal and illegal headquarters transitions, confirmed-before-prepare pickup rejection, legal preparing pickup, repeated one-time consumption, same-store/cross-store and role permissions, exact pickup audit/event writes, unchanged CRMEB/order/stock/product-quota/supply-chain/reward snapshots, and a true two-process pickup race with distinct operation keys.
- Strict-migration P1 is closed: the former English monthly menu labels exceeded `system_menus.menu_name VARCHAR(32)` and caused MySQL 8.0.46 default strict mode to leave tables and partial menus without a migration record. All inspected names are now within 32 characters; menu seeding is an idempotent `unique_auth` upsert and table creation is guarded.
- Official MySQL 8.0.46 strict-mode migration `run -> rollback -t 0 -> rerun -> duplicate run` passed on temporary database `yfth_monthly_benefit_strict_validation`. Run/rerun produced two tables and nine permissions, rollback removed them, and duplicate run remained stable. A constructed half-state with two tables, two permissions, and no migration record recovered to two tables, nine permissions, and one migration record.
- The concurrent pickup test confirmed one final benefit consumption, one quantity/counter update set, one pickup event, and one fulfillment/package-consumption audit. Long monthly idempotency keys are deterministically normalized only for the shared audit `request_id VARCHAR(64)` boundary; idempotency storage remains unchanged.
...
- 当前产品依据：`docs/YFTH_RELEASE_SCOPE_HQ_MALL_MEMBERSHIP_REFERRAL.md`
- 历史产品依据：`docs/PRODUCT_REQUIREMENTS.md` 与原始 DOCX，仅用于理解早期设计和已经落地的兼容实现；不得覆盖最新范围文档，发生冲突时以 Headquarters Mall 最新范围文档为准。
- 代码依据：`docs/PROJECT_INVENTORY.md`
- 结论类型：以下早期章节是基于当时真实仓库形成的历史开发前判断，不代表当前开发计划；后续更新章节记录真实完成事实。
...
在当时阶段，外部平台凭据完成轮换并验证前不得开始 5980 套餐、十个月权益、预约核销、加盟、产品额度或奖励台账等业务开发；这是历史安全门禁记录，不再代表当前开发顺序。

> [!NOTE]
> **以下第 1 至第 6 节是 2026-06-23 至早期开发阶段的历史差距判断。**其中部分模块已经完成，部分产品方向已被最新总部统一商城方案取代。“推荐开发顺序”和“第一项开发任务”均已执行或失效，不是当前下一步。当前开发顺序以最新范围文档中的阶段零、阶段一及后续阶段为准；当前下一步仍是完成文档复核和合并，合并后也必须由项目主控单独授权阶段零只读兼容与数据模型调查。
...
| 历史 5980 家庭康养套餐（兼容实现现已完成） | 购买后生成会员身份和十个月权益计划 | 普通商品、付费会员、其他订单 | 商品订单、`member_ship`、支付 | 套餐模板、套餐实例、权益计划、电子协议 | 当时建议新建套餐/权益域并复用支付订单；现作为稳定兼容实现保留 | 高 | V1.0 |
...
- 5980 套餐与十个月权益：当时判断不应简单做成普通商品或付费会员；对应套餐实例和权益计划现已完成并作为稳定兼容实现保留，不是新的开发建议。
...
以下顺序属于早期建议，其中多项已经完成，其余已失效或由最新范围文档重新排序；不得作为当前下一步。

1. 建立御方通和业务基础域：身份、门店归属、套餐实例、权益计划、审计事件和迁移规范。
2. 改造移动端底部导航和公共首页/康养中心，只接入真实可用的商城与门店能力。
3. 打通 5980 套餐购买、支付回调、会员生效和十个月权益计划生成（历史步骤，现已完成并保留兼容）。
...
当时建议的第一项开发任务为：**御方通和业务基础域与迁移规范设计/落地**。该任务现已完成，不是当前下一步。
