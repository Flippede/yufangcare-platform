...
Monthly idempotency records retain their normalized `monthly_benefit:{sha256}` key. Because that key is 80 characters while the shared audit table's `request_id` is `VARCHAR(64)`, audit writes use a deterministic SHA-256 request id whenever the source request id exceeds 64 characters. This keeps fulfillment and product-consumption audits valid under MySQL strict mode without changing shared audit or idempotency schemas.
...
- MySQL Community 8.0.46 isolated validation on temporary database `yfth_monthly_benefit_strict_validation`: the historical CRMEB baseline alone was imported with its compatibility session, while every YFTH `migrate:run`, `migrate:rollback -t 0`, rerun, duplicate run, and recovery run retained MySQL's default strict SQL mode. All ten inspected menu names are within `VARCHAR(32)` (current maximum 8 characters), and the former `Data too long for column 'menu_name'` failure is closed.
- Migration lifecycle evidence: run created two fulfillment tables and nine distinct monthly permissions; rollback removed both tables and all nine permissions; rerun restored them; duplicate run did not duplicate permission rows. A constructed half-state with two tables, only two permissions, and no `20260712100000` migration record recovered to two tables, nine permissions, and one migration record, then remained stable on another run.
...
- True concurrency evidence: two independent PHP worker processes use distinct operation keys against one `preparing` pickup. Both finish safely, while final item consumption, quantity zeroing, period/package counter increments, pickup event, fulfillment audit, and package-benefit consumption audit each occur once; both normalized idempotency records remain traceable.
