...
该 current 表明确**不包含** `source_unique_key`，也不建立任何 source key 唯一索引。`source_type/source_id` 只保存当前权威状态的来源摘要，不承担事件幂等唯一性；current 唯一性由 `UNIQUE(uid)`、状态、`authority_version`、业务事务和同版本事件共同保证。禁止给 current 增加 source key、用 current 来源字段阻止后续合法状态变化，或覆盖 current 来源摘要来代替历史事件。
...
事件只追加，不更新原事件，不物理删除。该表保存 event-specific `source_unique_key`，用于同一 attribution 业务事件来源的幂等：同一来源可按不同 event_type 产生多条合法事件，同一来源 + 同一 event_type 只能产生一次。不适用时必须为 `NULL`，不能用空字符串制造全局冲突。
...
current 中的 `source_unique_key` 只保存 `relation_created` 的可信创建来源，relation 创建成功后不可修改。paused、resumed、closed、invalid 均不得更新它；closed/invalid 后仍永久保留该历史来源键。该字段只阻止同一可信来源重复创建 relation，不承担后续状态事件幂等。
...
新关系的第一条 event 固定 `relation_version=1`, `event_type=relation_created`。current relation 与 version-1 event 缺一不可；event 失败则关系创建整体回滚。该 event 表保存 event-specific source key，`relation_created/relation_paused/relation_resumed/relation_closed/relation_invalid` 分别按 event_type 生成摘要；同一来源可产生不同 event_type，相同来源 + 相同 event_type 不得重复。后续状态变化和同版本 event 必须同事务；幂等重放返回原业务结果，不重复写 authority event。
...
三张实际存储 `source_unique_key` 的表统一使用域内 canonical SHA-256 规则；attribution current 不存该字段：

- 存储为 64 字符小写十六进制 SHA-256 摘要，字段可空。
- 无可信业务来源时必须写 `NULL`；禁止空字符串默认值。
- `source_type` 必须来自服务端 allowlist，`source_id` 必须服务端规范化；客户端提交的 `source_unique_key` 必须明确拒绝。
- 不直接保存客户端长字符串，不包含昵称、手机号、自由文本或私有凭证。
- source key 只在各自表内唯一，不要求跨表全局唯一。

四表职责冻结为：

| 表 | 是否存 `source_unique_key` | 职责 |
| --- | --- | --- |
| `yfth_hq_customer_attribution_current` | 否 | 当前归属状态和来源摘要；无 source key 索引 |
| `yfth_hq_customer_attribution_event` | 是 | attribution 事件来源幂等 |
| `yfth_hq_active_referral_current` | 是 | 仅 `relation_created` 来源唯一，创建后不可修改 |
| `yfth_hq_active_referral_event` | 是 | referral 状态事件来源幂等 |

canonical 输入顺序冻结为：

- attribution event：`hq_attribution_event|{event_type}|{source_type}|{canonical_source_id}`
- referral current 创建来源：`hq_active_referral_relation|relation_created|{source_type}|{canonical_source_id}`
- referral event：`hq_active_referral_event|{event_type}|{source_type}|{canonical_source_id}`

例如 `hq_attribution_event|attribution_created|active_referral|123` 与 `hq_attribution_event|attribution_unassigned|store_termination|456`。不同 domain 前缀保证同一来源可以合法产生一条 attribution event、一条 referral relation 和一条 referral event；event_type 参与事件摘要，因此同一来源可依次产生 created 和 closed 等不同事件，但相同来源加相同 event_type 不得重复。

`canonical_source_id` 规则：

- 数值主键转为无前导空格、无前导零的十进制字符串（数值 0 仅在对应 source 明确允许时使用）。
- Stage 1A 生产代码不得支持任意复合 source，只定义 canonicalizer 接口和固定格式规则；复合来源由未来场景服务按冻结字段顺序生成，各服务不得自由拼接。
- Stage 1A 测试仅允许测试命名空间/测试配置中的 source type allowlist；测试 source type 不进入生产 allowlist。未登记 source_type 必须 fail closed。
- 真实会员、套餐、推荐绑定和接管 source allowlist 在各自未来阶段分别冻结；调用方不得传入自由拼接的复合 source 字符串。
- Stage 1A contract check 必须断言 domain、event_type、source_type 与 source id 顺序，并验证相同输入摘要稳定、跨域不冲突。

migration 字段冻结为 `CHAR(64) CHARACTER SET ascii COLLATE ascii_bin NULL`，保存 64 位小写十六进制 SHA-256，并建立表内唯一索引；不适用来源写 `NULL`，允许多条无来源事件，空字符串不得入库。该摘要不输出给任何 API 角色，包括总部审计角色。

Stage 1A contract check 还必须验证：摘要始终为 64 位小写十六进制；字段使用 ASCII binary collation 或等价严格区分规则；相同输入摘要相同；不同 domain/event_type 不冲突；客户端 source key 被拒绝；NULL 可多行；空字符串被拒绝或规范化失败且不得入库。
...
- Stage 1A 使用一个主 migration 管理四表，创建顺序固定为 attribution current -> attribution event -> referral current -> referral event。
- 不使用数据库外键，但仍遵循该依赖顺序。
- migration 不创建 `system_menus`/API 权限，不 seed 业务数据或测试 fixture。
- 半迁移恢复逻辑必须校验完整列、类型、字符集、collation、索引和现有数据签名。
- 若实现时发现单 migration 无法安全表达 forward repair，不得擅自拆分；必须在 Stage 1A 报告中说明并由项目主控决定是否追加架构审核。

### 9.6 migration record 与半迁移恢复四种路径

| migration record | schema 状态 | 冻结处理 |
| --- | --- | --- |
| 不存在 | 四表均不存在 | 正常按依赖顺序创建四表，完成后由 runner 写 migration record |
| 不存在 | 存在部分兼容表/索引 | 先完整只读签名和数据检查，只补缺失表与安全缺失索引；不覆盖数据、不重写版本、不删冲突行；无法安全恢复立即失败 |
| 已存在 | schema 完整 | 正常 no-op，不重复修改表；duplicate-up 必须由隔离 test harness 直接调用或执行等价幂等路径验证，不能只凭 `migrate:run` 跳过记录宣称通过 |
| 已存在 | schema 不完整 | 阻断发布；不得删除/伪造 migration record 或手工改生产表；必须新增经架构审核的 forward repair migration，且只补兼容结构、不覆盖业务数据 |

MySQL DDL 不是完整事务化。“整体失败”表示检测异常后立即停止后续 DDL，不承诺自动回滚已经成功的 DDL。已形成的兼容半状态由同一 migration 的后续幂等恢复处理；migration record 已存在但 schema 不完整时只能由独立 forward repair migration 处理。每一步必须先检查依赖与签名，再创建表或索引。

其他迁移要求：

- 字段长度冻结建议：`event_no VARCHAR(48)`, `relation_no VARCHAR(48)`, `status VARCHAR(24)`, `status_reason_code VARCHAR(64)`, `request_id VARCHAR(64)`；三张存储 source key 的表使用 nullable `CHAR(64) CHARACTER SET ascii COLLATE ascii_bin`。
...
- duplicate-up/半迁移恢复可在同名表列签名完全符合时补建缺失表或已知缺失索引；缺失唯一索引只在现有数据满足约束时补建。
- 禁止覆盖业务行、重置状态、重写版本、清空冲突数据、修改不匹配列类型、自动删除重复行，或只因 `hasTable()` 为 true 就跳过签名校验。
- 同名表结构、现有数据或半迁移状态无法安全恢复时，migration 必须停止后续 DDL，不得修改旧 YFTH 表；migration record 已存在但 schema 不完整时执行 forward repair 门禁。
...
- 使用官方 MySQL Community 8.0.46 strict mode 隔离临时数据库完成 migration run/rollback/rerun、直接 duplicate-up/等价幂等路径和四种半迁移恢复验证。
- 并发验证必须使用两个独立 PHP 进程和两个独立数据库连接；不得以 SQLite、MariaDB 或单进程顺序调用替代。
- 必须覆盖：两店首次归属同一 UID、两个 referrer 竞争同一 referred、A 推荐 B 与 B 推荐 A、同一 referrer 并发处理多个 referred、source key 冲突、event version 冲突、deadlock/lock-wait retry 和同一 processing ownership。
...
- foundation contract。
- package benefit contract 与 real-flow。
- referral reward contract 与 real-flow。
- franchise customer contract 与 real-flow。
- service appointment contract 与 real-flow。

至少运行 contract/source guard：store workbench、monthly fulfillment、supply chain、product quota、franchise opening。

某项真实环境无法执行时不得虚构通过，必须记录阻塞原因并由项目主控判断是否阻塞 Stage 1A 验收。Stage 1A 不要求 Redis、真实 HTTP、admin build、H5、mp-weixin 或浏览器验证；这些属于 Stage 1B 或后续动态码/页面阶段。CRMEB 主链 diff 必须为零。
