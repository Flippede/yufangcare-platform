...
- Runtime: portable PHP 7.4.33, file cache, and a fresh isolated MySQL Community Server 8.0.46 instance on local port `33311`. No production MySQL, Redis, service or credential was used.
- Stage 1B permission migration passed run/down/up, rollback `-t 0`, rerun, duplicate-up, exact restoration, role-rule preservation, duplicate `unique_auth`, wrong `api_url/methods/auth_type/parent/path`, recorded missing permissions and down-conflict counterexamples.
- Stage 1B real HTTP flow passed 168 five-table before/after snapshot checks. Coverage includes user/store/HQ views, 15 independent Admin account shapes across six endpoints, strict parameters, recursive DTO restrictions, role revocation, store disablement, and actual attribution/referral corruption with fail-closed or headquarters-governance behavior.
- Stage 1A contract, source guard, migration lifecycle and real flow passed after extracting the shared validator, including real two-process competition, lock-wait retry and deadlock retry.
- The executable frontend stale-state test passed Admin out-of-order requests, permission/audit loss, tab isolation, store switching, role/store invalidation, teardown and user switching. Existing multi-role shell and request fallback checks also passed.
- Clean external outputs completed for Admin production, H5 production and mp-weixin production. This run observed Admin `606 files / 39,683,700 bytes`, H5 `349 files / 9,038,321 bytes`, and mp-weixin `1,234 files / 7,510,711 bytes`.
- Those file and byte counts are **environment evidence only**, affected by source maps, dependency caches, output cleanup and tool versions. They are not stable product contracts. Repeatable acceptance is successful build completion, required entry/page/chunk presence, no new compile error, no sensitive data, no committed output and no framework upgrade.
- The HBuilderX Node 22 attempt failed before compilation because its LibSass ABI binary was unavailable; the frozen compatible Node 18.20.8 runtime was then used successfully without installing or upgrading dependencies.
- No production deployment, production migration, server change or WeChat upload occurred. The feature branch remains unmerged and awaits independent read-only architecture review.
...
Stage 1B 第一次独立 Architecture Auditor 审核结论为 B，有条件通过；本轮整改和可重复验证已完成，但尚未经过整改后的独立复核，尚未合并 `main`，也未部署。下一门禁只能是独立只读架构复核。
