...
- 当前公开会员套餐初始价格为 9800 元，价格由总部通过套餐规则新版本维护，并同步专用 CRMEB 商品/SKU；历史订单价格快照不可改写。
- 0.1 元无支付模拟入口已退出公开购买链路。用户统一经过套餐列表、详情、门店、协议、CRMEB 下单与支付。
- 永久会员资格只需首次成功激活，但套餐允许重复购买。每笔已支付订单分别创建套餐实例和权益计划，不能因已有会员资格隐藏购买入口。
- 首次有效一级推荐转化仍在关系关闭前冻结 15%/25%/60% 序号、比例与金额；C1 可在佣金页面查看脱敏受邀人、比例、金额和状态。
