...
## Current Fact Snapshot - Headquarters Mall Stage 0 Architecture Review P1 Closure
...
- Attribution current states are frozen: `active` keeps `store_id>0` and allows same-store idempotent reuse; `paused` keeps `store_id>0` and continues occupying the UID authority; `unassigned` uses `store_id=0` and allows a later trusted first attribution; `closed` uses `store_id=0` and cannot be reopened by ordinary business paths.
