...
- The candidate now joins the existing real paths: headquarters versioned package/referral-rule configuration; C1 permanent membership and one active invite; C2 accepted B1 attribution; paid valid CRMEB ordinary-main-order candidate creation; C2 package activation/referral closure/15-25-60 candidate creation; B1 confirmation and immutable offline-settlement record; and C1/B1/headquarters role-scoped candidate reads.
- The isolated MySQL 8.0.46 core-flow check passed this full service-level chain. It also proved duplicate-safe candidate/settlement handling, a closed referral blocks later ordinary-mall candidates, and the path does not write CRMEB balance, brokerage, points, distribution, or withdrawal fields. It is not a browser, real-payment, or production-service test.
