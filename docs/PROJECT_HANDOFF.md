...
- Feature commit `7be42207f8a72262b02471951e605d2108b060b8` was fast-forward merged from `codex/yfth-customer-service-points-v1` into `main` and deployed to `https://yfth.top` on 2026-08-04. Production backup: `/www/backup/yfth-customer-service-points-20260804-203000`; retained release: `/www/releases/yfth-customer-service-points-7be42207-20260804-203000`.
...
- Production deployment used an idempotent forward-only migration equivalent to `20260804100000` because the historical duplicate version prevents the standard runner from starting. All five prefixed tables and required indexes were verified, the migration record was written, `.env` SHA-256 remained unchanged, and the H5/Admin authenticated browser flow displayed member points, points policy controls and customer-service assignment controls.
...
- 采购分润以采购单商品实付金额为基数，不含运费；下单时冻结规则版本、县/地/省/大区/平台五级链路和比例，收货入库后确认分润。初始比例为 20%/10%/5%/3%/1%，总部可按职级调整为包括 0% 在内的值。
- 合伙人工作台已展示采购收益、退款冲正、县级开店服务收益和平台加权分红。开店服务收益当前仅县级合伙人启用，初始金额为每个有效开店事实 17600 元；上级职级保留配置字段且初始为 0。
- 缺少显式招商来源时，系统按门店区域、服务区域优先级、有效开店负载、任职时间和 UID 稳定选择就近县级合伙人；已存在有效门店绑定或开店来源时不重新分配。
- 平台董事采购收益比例可设为 0；平台总采购业绩加权分红独立配置，初始池比例为 1%，按有效开店业绩权重生成月度批次。采购分润、开店服务收益和平台分红均为业务台账，不代表已完成真实打款。
- 新增采购分润快照/流水、开店收益流水、平台分红批次/明细及合伙人服务区域表；唯一约束和事务幂等防止重复收货、退款、开店事实或重复批次造成重复收益。
- 本地已通过 PHP 7.4 语法、65 项供应链契约、41 项采购分润契约、MySQL Community 8.0.46 migration run/rollback/rerun/duplicate run、采购分润真实流、供应链重复发货/收货/流水守卫、Admin/H5/mp-weixin 生产构建及既有多角色/请求降级契约。
- 既有供应链 V1 尚无采购售后页面或退款入口。本轮实现并验证了按采购单金额反冲的幂等服务边界，但未虚构一个不存在的采购售后产品流程。
- 当前文档快照尚未执行 main 合并、生产 migration 或生产部署；最终 Git 和生产事实应以本轮提交、快进合并和发布后的真实结果为准。
...
- Migration health derives the configured prefix and migration-table name, then checks `information_schema` for migration records, tables, columns, unique indexes and permissions. It is healthy only after a complete migration; partial, unmigrated, missing-index and missing-permission states fail closed.
- Mock profit sharing now requires all of `APP_ENV=testing`, explicit test mode and the isolated-database marker. Local and production configurations remain fail closed without a trusted provider.
- Actual final validation passed with PHP 7.4.33 and isolated MySQL Community 8.0.46: PHP syntax; automatic-commission and package-referral contracts; migration run, targeted rollback, rerun, duplicate run, actual missing-index and missing-permission rejection/restoration; package 15/25/60 accrual/closure/idempotency; ordinary C1/B1 commission and rule priority/snapshot; CRMEB legacy-brokerage exclusion; negative carry; itemized refund matrix; signed callback/provider checks; C1 same-store settlement; Admin, H5 and mp-weixin production builds; sensitive scan; and `git diff --check`.
- No main merge, production deployment, production migration, production database access or real WeChat profit-sharing call has occurred at this documentation snapshot. The feature is ready for the requested controlled fast-forward merge and production release; real profit sharing remains fail closed until a trusted production provider is configured.
...
- Final frozen product rule removes the entire B1 withdrawal model. There is no B1 withdrawal request, amount input, frozen withdrawal balance, withdrawal-success state or FIFO withdrawal allocation.
- Existing Stage 2 package 15%/25%/60% snapshots, Stage 3 mall order events, observation-day rules including 0-day mode, automatic credit and refund reversal remain. CRMEB product/order/payment/refund state machines are not rewritten.
- B1 accounts expose only unsettled amount, settled amount and settlement details. Headquarters groups immutable store commission ledgers into cycle batches with `pending/processing/settled/exception` states; successful callback moves the projection from unsettled to settled.
- WeChat profit-sharing is a reserved adapter, not a live integration. The model already freezes the B1 receiver, merchant subject, amount, internal/WeChat batch identifiers, status and idempotent callback facts so a later adapter does not require an account-model rewrite.
- C1 continues to submit settlement to its authoritative B1. The same-store manager or staff records offline completion; headquarters does not participate. Cross-store processing fails closed, and moving C1 available funds to requested/frozen state does not double-count the B1 pending responsibility.
- Commission remains isolated from CRMEB `now_money`, points, brokerage, distribution, wallet, withdrawal and `user_bill`, and cannot pay mall, package or purchase orders.
- Migration `20260720200000_create_yfth_automatic_commission_accounts_v1.php` now defines 12 tables including C1 settlement, B1 receiver, cycle batch, immutable batch items, reserved profit-sharing returns and callback facts. A refund after a completed settlement records a traceable negative adjustment for the next cycle without rewriting the completed batch. The read-only legacy report remains a production migration gate and performs no automatic backfill.
- Detailed design and evidence: `docs/YFTH_AUTOMATIC_COMMISSION_SETTLEMENT_V1.md` and `docs/YFTH_AUTOMATIC_COMMISSION_SETTLEMENT_RUNTIME_VALIDATION.md`.
...
- Production `https://yfth.top` serves functional commit `ce6c60e6fda2ef678074b2600bf313dab8b5ddfb`. Forward migration `20260719150000_create_yfth_account_closure_v2.php` is `up`, and `USER_ACCOUNT_CLOSURE_ENABLED=true` was restored only after deployment health checks and the dedicated TEST-account smoke passed.
- V2 uses explicit business-domain delete/anonymize matrices instead of the former full-schema UID-column scan. Login/profile/session/private data and current membership, attribution, referral, invitation and operating-role projections are removed; legally or operationally necessary order, payment, refund, fulfilment, reward, franchise/opening, partner-performance, audit, idempotency and authority facts are retained only as de-identified history under a random irreversible closure subject number.
- User self-service and headquarters-assisted closure share the same locked transaction, blocker checks and irreversible core service. Self-service requires password or reliable existing identity verification, agreement acceptance and the exact phrase `确认注销`; headquarters requires headquarters scope, dedicated permissions, a reason of at least four characters and cannot bypass unsettled funds, orders, contracts, store responsibility or risk freezes.
- Successful closure immediately revokes indexed Redis tokens and known sessions. The old account cannot log in or be restored. The same phone, account, openid and unionid can create or bind a fresh UID, and the new UID does not inherit old orders, package membership, store attribution, referral, rewards or operating roles.
- Isolated MySQL Community 8.0.46 verification covered migration run/targeted rollback/rerun, all required blockers, user and headquarters flows, C1/C2 relation handling, anonymous retained history, audit reliability, immediate token invalidation, phone/account/WeChat re-registration, no historical inheritance, idempotency and transaction rollback without partial deletion. PHP 7.4 syntax, focused contracts, Admin production build, H5 production build, mp-weixin production compile and `git diff --check` passed.
- Production verification used dedicated synthetic TEST accounts only. It confirmed immediate old-token rejection, cache/index removal, old identity de-identification, fresh phone/account/openid/unionid registration with new UIDs, no membership/attribution/role/order inheritance, persisted closure subjects and idempotent repeat. No real production user was deleted; no real payment, SMS or WeChat operation was performed.
- Production browser verification confirmed the H5 closure page renders the V2 explanation, blockers, security/agreement/confirmation flow and preflight state, and the headquarters `用户经营身份` page remains available with the `账号销户` operation. Homepage, Admin and closure routes return HTTP 200; the V2 Admin and H5 assets are present; no recent persistent fatal error or 5xx was found.
- Complete pre-cutover backup: `/www/backup/yfth-account-closure-v2-20260718-184652`; retained previous application directory: `/www/wwwroot/CRMEB-master.pre-account-closure-v2-20260718-190207`; pre-disable environment backup: `/www/backup/yfth-account-closure-v2-disable-20260718-165018/.env.before`. Backups include application, compressed full database dump, configuration/service snapshots, permissions and checksums and remain outside the Web/Git directories.
- Rollback is an explicit operational action: temporarily disable closure, stop only the YFTH Queue/Timer/Workerman units, atomically restore the retained previous application directory and saved `.env`, restore the full database dump only when data rollback is explicitly required, then reload PHP/Nginx and restart the YFTH units. Production rollback must never use migration target `0`.
- Final `main` and `origin/main` commit should be read from real Git after this documentation closure commit and push. Production intentionally remains on the functional commit above because the final follow-up commit changes documentation only.
...
- Production `https://yfth.top` now runs the reviewed functional release through commits `7172cda7` and `bcdf74b9`. Forward migration `20260719140000_add_franchise_application_approved_store.php` is `up`; application `FA202607181032090000047999` is bound to active store `北京房山张旭加盟店` and applicant UID 4 has one active `store_manager` role for that store. The verified pre-release backup is `/www/backup/yfth-franchise-manager-auto-20260718-141216`, and the retained release is `/www/releases/yfth-franchise-manager-auto-7172cda-20260718-141216`.
- Production browser verification confirmed the headquarters row displays `店长已生效`; public H5 and Admin return HTTP 200. No payment, refund, SMS, WeChat operation, production rollback, or unrelated service change was performed. Final `main` and `origin/main` should be read from real Git after this documentation closure commit.
...
- Focused verification passed with PHP 7.4.33: changed PHP syntax, 186-assertion franchise-application contract, 135-assertion legacy-opening contract, 65-assertion supply-chain contract, isolated MySQL 8.0.46 migration run/targeted rollback/rerun, Admin production build, and `git diff --check`.
- Production `https://yfth.top/admin/` now runs functional commit `4b98c24d80e27756444fb08794d23a931bce2763`. Forward migration `20260719130000_simplify_franchise_review_and_localize_admin.php` is `up`; the affected code, complete Admin assets, `.env`, permissions, and full MySQL 8 database dump are retained at `/www/backup/yfth-franchise-review-simplified-20260718-120425`, and the release is retained at `/www/releases/yfth-franchise-review-simplified-20260718-120425`.
- Authenticated production browser verification confirmed application `FA202607181032090000047999` is visible as `待总部确认`, with only `详情`, `同意加盟`, and `驳回申请` actions. The approval dialog requires a reason and directs headquarters to the existing user-role page after approval. The dialog was cancelled during verification, so the production application was not approved, rejected, reassigned, or otherwise mutated. Existing signed-in Admin sessions may retain their login-time menu labels until the next login; the production menu rows themselves are localized and the legacy opening page is hidden.
...
- Production `https://yfth.top` now runs functional commit `fa9502d4780b026999e5be86bfa368e1eea93f13` plus Admin hint fix `802b1560d38535510743dfa2da8e9d1ef20bcf0d`. The permission migration is `up`, both hidden APIs are registered, and a signed-in browser verified the visible grant action, all five ranks, the province-to-regional parent requirement, and the parent-free platform director. Full database and affected-code backup: `/root/yfth-backups/partner-manual-grant-20260717-175013`; pre-hint-fix Admin backup: `/root/yfth-backups/partner-manual-grant-ui-fix-20260717-180131/admin-fa9502d`.
...
- 开店奖励先形成待确认额度，由总部确认后才记入绑定门店的可用额度；重复确认幂等。商品额度采购支持预占、入库核销、拒绝释放、售后退回和退回冲正；采购页面支持商品额度与在线金额混合展示。
