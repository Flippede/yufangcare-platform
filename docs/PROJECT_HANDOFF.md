...
- Initial P1 root cause closed in the prior hardening round: headquarters `complete` could previously move `confirmed` directly to `completed`, which immediately consumed the monthly product benefit and bypassed preparation, shipment, or pickup.
- Remaining P1 found by architecture re-review: store `pickup_confirm` still accepted `confirmed`, so a same-store operator could bypass headquarters preparation and consume the monthly product benefit through `confirmed -> pickup_confirm -> completed`.
- P1 closure: express delivery must follow `pending_confirm -> confirmed -> preparing -> shipped -> completed`; headquarters `complete` now accepts only `shipped` for express delivery and cannot complete `confirmed` or `preparing` rows.
- P1 closure: self pickup must follow `pending_confirm -> confirmed -> preparing -> pickup_confirm -> completed`. Store `pickup_confirm` accepts only `preparing`; the store workbench hides its confirmation button for `confirmed`, and headquarters still cannot bypass the same-store pickup action.
- Completion boundary: final product benefit consumption updates only `yfth_benefit_item`, `yfth_benefit_period`, and `yfth_package_instance` counters; repeated `complete` or repeated `pickup_confirm` does not consume twice; it does not create CRMEB orders or mutate CRMEB stock.
- P2 closure: service-layer claim payload validation now rejects `active_key`; express shipping requires both `delivery_company` and `delivery_no`.
...
- Validation executed: PHP syntax checks for every PHP file changed from `main`; monthly benefit contract check with 97 assertions; monthly benefit real-flow source guard; MySQL 8.0.46 isolated migration run / rollback -t 0 / rerun / duplicate run and unique-index guards on temporary database `yfth_monthly_benefit_validation`; isolated MySQL service-level real-flow covering `confirmed` pickup rejection before preparation, unchanged benefit item/quantity/period/package counters, no rejected event/final-consumption audit, legal `preparing -> pickup_confirm -> completed`, repeated pickup one-time consumption, exact pickup event/audit, cross-store rejection, customer/service-mentor rejection, store-staff/store-manager/franchisee permission, and unchanged CRMEB/order/stock/product-quota/supply-chain/reward snapshots; package benefit, service appointment, supply chain, and product quota contract checks; admin production build; uni-app multi-role shell and request fallback Node checks; H5 production build; mp-weixin production compile; `git diff --check main..HEAD`.
- Isolated migration environment fact: the existing monthly menu seed contains English `menu_name` values longer than CRMEB's 32-character column. Default MySQL strict mode rejects that pre-existing seed; validation reused the project's historical compatibility mode `NO_ENGINE_SUBSTITUTION` as required by the task boundary and did not modify the migration. This remains visible evidence for the next architecture re-review rather than being reported as a strict-mode pass.
- H5 and mp-weixin production compilation were executed with the repository-external HBuilderX `uniapp-cli` plus Node 18.20.8; no WeChat upload was performed.
...
- Next gate: run read-only architecture re-review for this remaining-P1 closure and the recorded strict-mode migration compatibility fact before any main merge decision.
