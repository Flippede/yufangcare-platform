...
- P1 root cause closed in this round: headquarters `complete` could previously move `confirmed` directly to `completed`, which immediately consumed the monthly product benefit and bypassed preparation, shipment, or pickup.
...
- P1 closure: self pickup uses the explicit store workbench `pickup_confirm` action from the same active store, records event type `pickup_confirm`, and completes the fulfillment without allowing headquarters to bypass pickup confirmation from `confirmed` or `preparing`.
...
- Validation executed: PHP syntax checks for new/modified PHP files; monthly benefit contract check; monthly benefit real-flow source guard; MySQL 8.0.46 isolated migration run / rollback -t 0 / rerun / duplicate run and unique-index guards on temporary database `yfth_monthly_benefit_validation`; isolated MySQL service-level real-flow covering claim, duplicate claim, payload mismatch, non-owner/unopened/expired/frozen/closed/refunded rejection, user cancel, headquarters legal and illegal transitions, shipping field validation, repeated complete one-time consumption, same-store pickup confirmation, cross-store rejection, customer/service-mentor rejection, store-staff/store-manager/franchisee permission, audit/event writes, and adjacent module boundary snapshots; package benefit, service appointment, supply chain, and product quota contract checks; admin production build; uni-app multi-role shell and request fallback Node checks; H5 production build; mp-weixin production compile; `git diff --check main..HEAD`.
...
- Next gate: run read-only architecture re-review for this P1/P2 closure before any main merge decision.
