...
## Current Fact Snapshot - Automatic Commission Balance And Withdrawal V1 Development

- Development branch: `codex/yfth-auto-commission-balance-withdrawal-v1`; base `main/origin/main`: `e0dbcdee4b87b2955a964c385a3fd8a4a61e7bc1`. This stage is not merged to main and is not deployed; final feature commit must be read from Git after the branch commit and push.
- Existing Stage 2 package 15%/25%/60% snapshots, Stage 3 mall order events, Stage 4 settlement facts, Stage 1A authority, store workbench context, audit and idempotency capabilities are reused. CRMEB product/order/payment/refund state machines are not rewritten.
- Package rewards and normal-mall commissions are separate. Package activation credits only C1 plus the responsible B1's C1-proxy bucket; mall completion credits C1 and B1-own according to an immutable product/category/global rule snapshot. C2 membership activation closes the old active referral; frozen earlier orders still settle, while later orders no longer credit the old C1.
- Normal commission follows `paid -> completed -> observing -> credited`; zero-day rules credit in the completion event, while `yfth:commission-settle` is the compensation scanner. Published rule versions freeze integer basis points, scope precedence and observation days for each order.
- The user-facing `账户余额` surface combines CRMEB commerce balance presentation with a restricted YFTH commission subledger. Commission does not write `now_money`, brokerage, points, distribution or `user_bill`, cannot pay for mall/package/purchase orders, and is withdrawable only through the authoritative B1 relationship.
- B1 accounts are keyed by `store_id` and separate store-own, C1-proxy, headquarters-frozen, headquarters-withdrawn and C1-pending amounts. Headquarters withdrawals automatically allocate immutable credit lots FIFO; verified 50/180/400 yuan cases split exactly across 100 yuan own plus 300 yuan proxy funds. A headquarters withdrawal never reduces C1 personal balance or C1 pending.
- C1 withdrawal is a two-state B1 offline-payment record. Both store manager and store staff may complete an authorized same-store C1 withdrawal; cross-store operations fail closed. B1 headquarters withdrawal is `reviewing/success`, uses an encrypted default settlement-account snapshot and database locks, idempotency and bounded deadlock retry to prevent overdraw.
- Full and partial refunds append linked negative ledgers. Observing commissions are cancelled before credit; credited commissions can become negative and future income offsets the deficit. Original accrual and settlement facts remain immutable.
- Migration `20260720200000_create_yfth_automatic_commission_accounts_v1.php` adds 10 tables, package observation days and headquarters permissions. Isolated MySQL Community 8.0.46 run/targeted rollback/rerun passed. The read-only `yfth:commission-legacy-report` command is required before production migration and does not automatically settle legacy pending/confirmed candidates.
- PHP 7.4 syntax, focused contracts, Stage 3 and automatic-commission real flows, exact FIFO and concurrent-withdrawal checks, Admin/H5/mp-weixin production builds and `git diff --check` passed. The historical Stage 2 full-flow script still encodes the superseded manual-pending product behavior and is explicitly not reported as passing; the independent review must confirm the new automatic-flow replacement evidence.
- Detailed architecture and evidence: `docs/YFTH_AUTOMATIC_COMMISSION_BALANCE_WITHDRAWAL_V1.md` and `docs/YFTH_AUTOMATIC_COMMISSION_BALANCE_WITHDRAWAL_RUNTIME_VALIDATION.md`.
- Next gate: independent architecture review only. Do not merge main, run production migration, backfill production data, deploy production, or enable real withdrawals before approval.
